<template>
  <div class="app-container">
    <!-- 对话框(添加 / 修改) -->
    <el-form ref="form" :model="form" :rules="rules" label-width="10vw" label-position="left" v-loading="loading">
      <el-form-item label="是否有纸质收据" prop="paperReceipt">
        <el-radio-group v-model="form.paperReceipt" @change="resetOrderPayImg()">
      ...
      <el-form-item prop="payDate">
        ...
        <el-date-picker clearable size="small" v-model="form.payDate" type="date" value-format="timestamp"
          placeholder="选择付款时间" :picker-options="pickerOptions" />
      </el-form-item>
      <el-form-item label="采购项目" prop="purchaseObjs">
        <el-col :span="10">
          <el-input type="textarea" :rows="3" v-model="form.purchaseObjs" placeholder="请输入物品名称，可输入多个" :maxlength="500"
            :show-word-limit="true" />
        </el-col>
      </el-form-item>
      <el-form-item label="用途" prop="usage">
        <el-col :span="10">
          <el-input type="textarea" :rows="3" v-model="form.usage" placeholder="请输入物品用途" :maxlength="500"
            :show-word-limit="true" />
        </el-col>
      </el-form-item>
      <el-form-item label="购买途径" prop="purBy">
        <el-select v-model="form.purBy" placeholder="请选择途径">
          <el-option v-for="dict in wayTypeDictData" :key="parseInt(dict.value)" :label="dict.label"
            :value="parseInt(dict.value)" />
        </el-select>
      </el-form-item>
      <el-form-item label="货币种类" prop="totalUnit">
        <el-select v-model="form.totalUnit" placeholder="请选择货币种类">
          <el-option v-for="dict in currencyTypeDictData" :key="parseInt(dict.value)" :label="dict.label"
            :value="parseInt(dict.value)" />
        </el-select>
      </el-form-item>
      <el-form-item label="总价" prop="totalPrice">
        <el-col :span="10">
          <el-input-number :rows="3" v-model="form.totalPrice" placeholder="请输入采购物品总价" :controls="false"
            :precision="2" />
        </el-col>
      </el-form-item>
      <el-form-item prop="exchangeRate">
        <span slot="label">
          <el-tooltip content="汇率数据是根据付款日期和货币种类，查询网址 https://tw.exchange-rates.org " placement="top">
            <i class="el-icon-question" />
          </el-tooltip>
          兑港币汇率
        </span>
        <el-col :span="10">
          <el-input-number :rows="3" v-model="form.exchangeRate" :disabled="true" :controls="false" />
        </el-col>
      </el-form-item>
      <el-form-item label="总价（港币）" prop="totalHkd">
        <el-col :span="10">
          <el-input-number :rows="3" v-model="form.totalHkd" :disabled="true" :controls="false" :precision="2" />
        </el-col>
      </el-form-item>
      <el-form-item prop="remark">
        <span slot="label">
          <el-tooltip content="请填写：1.设备用于研发什么项目；2.在项目中的用途 " placement="top">
            <i class="el-icon-question" />
          </el-tooltip>
          备注
        </span>
        <el-col :span="10">
          <el-input type="textarea" :rows="3" v-model="form.remark" placeholder="请介绍设备用于研发什么项目，以及在项目中的用途"
      ...
      <el-form-item label="订单截图" v-show="form.paperReceipt != undefined && !form.paperReceipt" prop="orderImg">
        <imageUpload v-model="form.orderImg" :limit="1" />
      </el-form-item>
      <el-form-item label="付款截图" v-show="form.paperReceipt != undefined && !form.paperReceipt" prop="payImg">
