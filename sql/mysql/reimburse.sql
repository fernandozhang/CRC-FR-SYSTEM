CREATE TABLE `ruoyi-vue-pro`.`bpm_reim_batch` (
  ...
  `reim_person_name` VARCHAR(64) NOT NULL COMMENT '报销人员名字',
  `cur_app_job` VARCHAR(64) COMMENT '当前审批任务',
...
CREATE TABLE `ruoyi-vue-pro`.`bpm_reim_purchase` (
  ...
  `total_unit` VARCHAR(8) NOT NULL COMMENT '付款金额单位(1rmb,2hkd,3usd)',
  ...
  `batch_id` BIGINT NOT NULL COMMENT '批次号',
